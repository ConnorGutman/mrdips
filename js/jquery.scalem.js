 ...
 */
var lineLimit = 25;
var messageContent = $("#alert-text").html();
var charCount = $("#alert-text").html().length;
if (charCount > lineLimit) {
  lineLimit = Math.ceil((charCount / 2)+5);
}
if (charCount > lineLimit) {
  var testEval = 'messageContent = messageContent.replace(/(.{1,' + lineLimit + '})(?:' + "&#92;" + 'n|$| )/g, "$1<br>");';
  messageContent = eval(testEval);
  $("#alert-text").html(messageContent);
  $("#alert-message").css("height", "40vh");
}
